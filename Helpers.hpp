...
namespace ECE141
{
  ...
  static Keywords gAlterTypes[] = {
      Keywords::add_kw, Keywords::drop_kw, Keywords::modify_kw};

  static std::map<std::string, bool> mUseCache{
      std::make_pair("Block", true),
      std::make_pair("Row", true),
      std::make_pair("View", true),
  };
  ...
  public:
    ...
    static std::string valueToString(Value theValue)
    {
      static char vtypes[] = {'b', 'i', 'd', 's'};
      char theType = vtypes[theValue.index()];
      switch (theType)
      {
      case 'b':
        return (std::get<bool>(theValue)) ? "True" : "False";
      case 'i':
        return std::to_string(std::get<int>(theValue));
      case 'd':
        return std::to_string(std::get<double>(theValue));
      case 's':
        return std::get<std::string>(theValue);
      default:
    ...
    static bool validateValues(std::vector<Value> &values, std::vector<Token> &valuetokens, std::vector<DataTypes> &valuetypes)
    {
      if (valuetokens.size() != valuetypes.size())
        return false;
      ...
      for (int i = 0; i < valuetokens.size(); i++)
      {
        ...
        case DataTypes::bool_type:
        {
          if (valuetokens[i].keyword == Keywords::true_kw || valuetokens[i].keyword == Keywords::false_kw)
          {
            Value theValue = (valuetokens[i].keyword == Keywords::true_kw) ? true : false;
        ...
        case DataTypes::int_type:
        {
          if (valuetokens[i].type == TokenType::number)
          {
            Value theValue = std::stoi(valuetokens[i].data);
            values.push_back(theValue);
            break;
          }
          return false;
        ...
        case DataTypes::float_type:
        {
          if (valuetokens[i].type == TokenType::number)
          {
            Value theValue = std::stof(valuetokens[i].data);
            values.push_back(theValue);
            break;
          }
          return false;
        ...
        case DataTypes::datetime_type:
        {
          if (valuetokens[i].type == TokenType::identifier || valuetokens[i].type == TokenType::string)
          {
            Value theValue = valuetokens[i].data;
            values.push_back(theValue);
            break;
          }
          return false;
    ...
    static StatusResult parseAttributeProp(Tokenizer &aTokenizer, Attribute &anAttribute)
    {
      switch (aTokenizer.current().keyword)
      {
      case Keywords::not_kw:
        anAttribute.nullable = false;
        ...
        break;
      case Keywords::integer_kw:
        anAttribute.field_type = DataTypes::int_type;
        aTokenizer.next();
        break;
      case Keywords::boolean_kw:
        anAttribute.field_type = DataTypes::bool_type;
        aTokenizer.next();
        break;
      case Keywords::float_kw:
        anAttribute.field_type = DataTypes::float_type;
        aTokenizer.next();
        break;
      case Keywords::datetime_kw:
        anAttribute.field_type = DataTypes::datetime_type;
        aTokenizer.next();
        break;
      case Keywords::varchar_kw:
        anAttribute.field_type = DataTypes::varchar_type;
        aTokenizer.next(2);
        anAttribute.field_length = std::stoi(aTokenizer.current().data);
        aTokenizer.next(2);
        break;
      case Keywords::auto_increment_kw:
        anAttribute.auto_increment = true;
        anAttribute.nullable = false;
        aTokenizer.next();
        break;
      case Keywords::primary_kw:
        if (aTokenizer.peek(1).keyword != Keywords::key_kw)
        {
          return StatusResult{invalidArguments};
        }
        anAttribute.primary_key = true;
        aTokenizer.next(2);
        break;
      default:
        return StatusResult{invalidArguments};
